...
const ISLAND_INTRO = {
  greetings: {
    ...
    story: 'Every great voyage begins with a wave! On Greetings Island you will learn the foundational signs used every day — from a friendly "Hello" to a heartfelt "Thank You". These signs open doors and start conversations.',
    description: 'Master the 5 most essential greeting signs in American Sign Language. Perfect for beginners stepping onto their first island.',
    objective: 'Complete all 5 phrase levels to unlock the Greetings Boss Challenge.',
    hint: 'Keep your hand clearly centered in frame and use smooth, deliberate movements.',
  },
  family: {
    title: 'Family Island',
    story: 'Deep in the emerald coves of Family Island, you will discover how ASL brings families closer. Learning "Mother", "Father", "Sister", and more lets you talk about the people who matter most.',
    description: 'Learn the signs for 5 core family members. These signs use handshapes near the face to show warmth and connection.',
    objective: 'Sign all family member phrases and take on the Family Boss Challenge.',
    hint: 'Most family signs are near your chin (female) or forehead (male) — position matters!',
  },
  colors: {
    title: 'Colors Island',
    story: 'Colors Island is a kaleidoscope of vibrant hues swirling in the ocean breeze. ASL uses clever hand shapes and movements to represent color — many are derived from the fingerspelled letter of the color.',
    description: 'Explore 5 vivid color signs. Colors in ASL often involve twisting or shaking their corresponding letter hand.',
    objective: 'Learn all 5 color signs and face the Colors Boss Challenge.',
    hint: 'Colors like Blue (B-hand) and Green (G-hand) involve a gentle wrist twist — practice the motion slowly.',
  },
  food: {
    title: 'Food Island',
    story: 'The rich aromas of Food Island drift across the sea! From "Eat" to "Apple", ASL food signs are intuitive and fun — many mimic the action of eating or the food itself. Your stomach will thank you!',
    description: 'Discover 5 essential food-related signs. Food signs in ASL are highly visual and often easy to remember.',
    objective: 'Complete all food phrase levels to unlock the Food Boss Challenge.',
    hint: 'Food signs often involve bringing your hand toward your mouth — keep the motion natural and fluid.',
  },
  animals: {
    title: 'Animals Island',
    story: 'Welcome to the wild shores of Animals Island! Here, ASL signs come alive — you will mimic whiskers, wagging tails, and flapping wings. Animal signs are among the most expressive and fun in the language.',
    description: 'Learn 5 expressive animal signs. Animal signs in ASL often recreate the most distinctive feature of each creature.',
    objective: 'Master all animal signs and challenge the Animals Island Boss.',
    hint: 'Use your whole hand and face for animal signs — expression makes the sign come alive!',
  },
};


export const STUDY_ISLANDS = STUDY_TOPICS.map((topic, index) => {
  ...
  const intro = ISLAND_INTRO[topic.id] || {
    ...
    objective: 'Clear all phrase levels to unlock the boss sentence challenge.',
...
const syncLegacyFields = (progress) => {
  const completedTopics = STUDY_ISLANDS.filter((island) => progress.islands[island.id]?.bossCompleted).map((island) => island.id);
...
const recomputeUnlockedIslands = (progress) => {
  ...
  STUDY_ISLANDS.forEach((island, index) => {
    ...
    if (progress.islands[prevIsland.id]?.bossCompleted) {
      unlocked.push(island.id);
    }
  });
...
const computeXpFromProgress = (progress) => {
  return STUDY_ISLANDS.reduce((acc, island) => {
    ...
    const levelXp = islandProgress.completedLevelIds.length * XP_PER_LEVEL;
    const bossXp = islandProgress.bossCompleted ? XP_PER_BOSS : 0;
...
export const isIslandCompleted = (progress, islandId) =>
  Boolean(progress?.islands?.[islandId]?.bossCompleted);
...
export const completeIslandLevel = (progress, islandId, levelId) => {
  ...
  const isBoss = levelId === island.bossLevel.id;
  ...
  if (isBoss) {
    ...
    totalXp += island.bossLevel.rewardXp;
