...
export default function Converse() {
  ...
  const [status, setStatus] = useState('Loading Converse...');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!userId) {
        setBootstrapError('You need to be signed in to use Converse.');
        return;
      }
      try {
  ...
  const clearProcessingTimers = useCallback(() => {
    processingTimersRef.current.forEach((id) => window.clearTimeout(id));
  ...
  const finishProcessingFeedback = useCallback((isSuccess, message) => {
    ...
    const t = window.setTimeout(() => setShowProcessingModal(false), isSuccess ? 900 : 1500);
  ...
  const advanceToNextPrompt = useCallback(() => {
    ...
    setCountdown(0);
    setStatus('Next — read the message and sign your reply.');
    setCurrentIndex((idx) => {
      const next = idx + 1;
      return next < prompts.length ? next : idx;
    });
  ...
  const submitCurrentFrames = useCallback(async (debugOverrideWord = null) => {
    ...
    setStatus(`Checking your sign for "${currentPrompt.expected_word.toUpperCase()}"...`);
    ...
    try {
      ...
      if (response.is_correct) {
        setCorrectPromptIds((prev) => { const n = new Set(prev); n.add(currentPrompt.id); return n; });
      }
      ...
      if (response.is_correct) {
        finishProcessingFeedback(true, `Correct · Closest: ${(response.matched_word || currentPrompt.expected_word).toUpperCase()} · ${Number(response.confidence || 0).toFixed(2)}`);
      } else {
        finishProcessingFeedback(false, `Wrong gesture · Closest: ${(response.matched_word || 'unknown').toUpperCase()} · ${Number(response.confidence || 0).toFixed(2)}`);
      }
    } catch (error) {
  ...
  const handleRecordToggle = useCallback(() => {
    ...
    if (stopFrame) frameBufferRef.current = [...frameBufferRef.current, stopFrame].slice(-FRAME_BUFFER_SIZE);

    if (frameBufferRef.current.length === 0) {
      setReadyToSubmit(false);
      setStatus('No frames captured — center your hand and try again.');
      return;
    }
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#020a1c', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Nunito',sans-serif", padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 420 }}>
          <h2 style={{ fontSize: 22, fontWeight: 900, margin: '0 0 10px' }}>Converse unavailable</h2>
          <p style={{ margin: '0 0 20px', color: 'rgba(255,255,255,0.7)' }}>{bootstrapError}</p>
  ...
  if (sessionSummary) {
    return <SummaryScreen summary={sessionSummary} onAgain={() => window.location.reload()} onExit={() => navigate('/dashboard')} />;
  }

  if (!currentPrompt) {
    return (
      <div style={{ minHeight: '100vh', background: '#020a1c', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Nunito',sans-serif" }}>
        <p style={{ color: 'rgba(255,255,255,0.6)', fontWeight: 700 }}>{status}</p>
      </div>
    );
  }
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#020a1c', fontFamily: "'Nunito', sans-serif" }}>
      ...
      <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', width: '100%', maxWidth: 1140, maxHeight: '100vh', borderRadius: 0, overflow: 'hidden', background: 'linear-gradient(165deg, #0c1f3d 0%, #081428 100%)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', background: 'rgba(0,0,0,0.25)', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          <button onClick={() => navigate('/dashboard')} style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <ArrowLeft size={16} color="white" />
          </button>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <MessageCircle size={16} color="#6ee7b7" />
            <span style={{ fontSize: 11, fontWeight: 900, color: '#6ee7b7', textTransform: 'uppercase', letterSpacing: '0.16em' }}>Converse</span>
          </div>
        ...
        <div className="cv-body" style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          {/* Camera side */}
          <div style={{ flex: 1, position: 'relative', background: '#040c18', minHeight: 420 }}>
            ...
            <div style={{ position: 'absolute', top: 14, left: 14, background: recording ? 'rgba(239,68,68,0.9)' : 'rgba(0,0,0,0.55)', backdropFilter: 'blur(8px)', borderRadius: 99, padding: '5px 12px', display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: recording ? 'white' : '#ef4444', animation: recording ? 'cv-blink 1s ease-in-out infinite' : undefined }} />
          ...
          {/* Prompt panel */}
          <div className="cv-panel" style={{ width: 380, flexShrink: 0, background: 'linear-gradient(180deg,#0e2347 0%,#091832 100%)', display: 'flex', flexDirection: 'column', padding: '22px 20px 18px', gap: 12, overflowY: 'auto', borderLeft: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ height: 4, borderRadius: 99, background: 'rgba(255,255,255,0.12)', overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 99, width: `${totalPrompts ? Math.round((correctCount / totalPrompts) * 100) : 0}%`, background: 'linear-gradient(90deg,#34d399,#22d3ee)', transition: 'width 0.4s ease' }} />
            </div>

            {/* NPC message bubble */}
            <div style={{ background: 'rgba(255,255,255,0.05)', border: '1.5px solid rgba(255,255,255,0.12)', borderRadius: 18, padding: '14px 16px' }}>
              <div style={{ fontSize: 11, fontWeight: 900, color: 'rgba(255,255,255,0.55)', letterSpacing: '0.16em', textTransform: 'uppercase', marginBottom: 8 }}>
                💬 NPC Says
              </div>
            ...
            {/* Result feedback */}
            <div style={{ borderRadius: 14, background: latestResult?.is_correct ? 'rgba(34,197,94,0.18)' : latestResult ? 'rgba(239,68,68,0.12)' : 'rgba(255,255,255,0.06)', border: `1px solid ${latestResult?.is_correct ? 'rgba(52,211,153,0.45)' : latestResult ? 'rgba(239,68,68,0.35)' : 'rgba(255,255,255,0.12)'}`, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {latestResult?.is_correct ? <CheckCircle2 size={16} color="#4ade80" /> : latestResult ? <AlertCircle size={16} color="#f87171" /> : null}
      ...
      <button
        ...
        title="How to use Converse"
        style={{
          position: 'fixed', bottom: 20, right: 20, zIndex: 100,
          width: 36, height: 36, borderRadius: '50%',
          background: 'rgba(110,231,183,0.15)', border: '1.5px solid rgba(110,231,183,0.4)',
          color: '#6ee7b7', fontSize: 16, fontWeight: 900, cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: "'Nunito',sans-serif",
          boxShadow: '0 2px 12px rgba(0,0,0,0.4)',
          transition: 'background 0.15s ease, transform 0.15s ease',
        }}
        onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(110,231,183,0.28)'; e.currentTarget.style.transform = 'scale(1.1)'; }}
        onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(110,231,183,0.15)'; e.currentTarget.style.transform = 'scale(1)'; }}
      >?</button>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          .cv-body { flex-direction: column !important; }
          .cv-panel { width: 100% !important; max-height: 50vh; }
        }
      `}</style>
    </div>
  );
...
function SummaryScreen({ summary, onAgain, onExit }) {
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at 30% 0%, #22d3ee 0%, #0369a1 60%, #041421 100%)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Nunito',sans-serif", padding: 24 }}>
      <div style={{ maxWidth: 460, width: '100%', background: 'rgba(255,255,255,0.08)', border: '1.5px solid rgba(255,255,255,0.18)', borderRadius: 24, padding: '28px 28px 24px', textAlign: 'center' }}>
        ...
        <h1 style={{ margin: '0 0 8px', fontSize: 26, fontWeight: 900 }}>Session Complete</h1>
        <p style={{ margin: '0 0 22px', color: 'rgba(255,255,255,0.75)', fontSize: 14, lineHeight: 1.5 }}>
          You finished a Converse session. Keep practicing to build fluency!
        </p>
